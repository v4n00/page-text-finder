...
const defaultText = 'This text box serves as the storage that the extension will search through for any text you select on any web page.</br></br>To disable the extension, click Clear and then Save.</br></br>The number input below specifies the buffer size, which is the number of characters that will be displayed after the successful word match. Change this number based on the content.';

// retrieve text from storage
chrome.storage.local.get(['textStorage', 'bufferSize'], (data) => {
	textBox.innerHTML = data.textStorage || '';
	bufferSize.value = data.bufferSize || 5000;
});
